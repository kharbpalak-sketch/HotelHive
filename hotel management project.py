...
Tablename=input("Name Of a Table To Be Created :")
...
print(" ")
print("-"*110)
print(" creating table which stores all the information of customer ")
print("-"*110)
mycursor.execute("use"+" "+ db)
Tablename1=input("Name Of a Table To Be Created :")
queryy="create table if not exists" +" "+ Tablename1 +"\
(NAME varchar(20) not null,\
...
Price_of_room varchar(10),\
Total_room_charge varchar(13),\
Resturent_charge varchar(20),\
Total_bill varchar(20),\
Room_no varchar(5),\
Customer_id varchar(5))"
print("Table"+" "+Tablename1+" "+"created successfully")
...
def Home():
   ...
   print("\t\t\t 6 To view particular Record\n")
   print("\t\t\t 7 To Delete particular Record\n")
   print("\t\t\t 0 Exit\n")
   print(" ")
...
def Booking():
        ...
        if ch==1:
            ...
            print(" Wrong choice..!!")
        query="insert into"+" "+Tablename+" "+"values('{}',{},'{}')".format(n,p1,a)
        mycursor.execute(query)
        mydb.commit()
...
def Payment():
            ...
            # not already done
             if p[n]==0:
                ...
                if ch=='y' or ch=='Y':
                    ...
                    print(" --------------------------------")
                    print(" Room number: \t",roomno[n])
                    print(" Customer id: \t",custid[n])
                    print(" --------------------------------")
    ...
    h=price[n]*day[n]
    i=rc[n]
    j=price[n]*day[n]+rc[n]
    k=roomno[n]
    l=custid[n]
    lst=[a,b,c,d,e,f,g,h,i,j,k,l]
    print(lst)

    sql = "INSERT INTO "+Tablename1+"(name,phone_no,address,check_in,check_out,room_type,"+\
          "Price_of_room,Total_room_charge,Resturent_charge,Total_bill,room_no,customer_id)"+\
...
# RECORD FUNCTION 
def allRecord():
    qr="select * from "+Tablename1
    mycursor.execute(qr)
    result=mycursor.fetchall()
    h=['NAME','PHONE_NO','ADDRESS','Check_In','Check_Out','Room_Type','Price_of_room',    'Total_room_charge','Resturent_charge','Total_bill','Room_no','Customer_id']
...
def customer():
    ...
    qrr="select * from "+Tablename1+" where Customer_id = "+cid
    mycursor.execute(qrr)
    result=mycursor.fetchall()
    h=['NAME','PHONE_NO','ADDRESS','Check_In','Check_Out','Room_Type','Price_of_room',
       'Total_room_charge','Resturent_charge','Total_bill','Room_no','Customer_id']
    print(tabulate(result,headers=h,tablefmt='psql'))
    n = int(input("0-BACK\n ->"))
    if n == 0:
       Home()       
...
def delete():
    cid=input("enter customer id")
    rn=input("enter room no.")
    qrr="DELETE from "+Tablename1+" where Customer_id = "+cid+" and "+" Room_no = "+rn
    mycursor.execute(qrr)
    mydb.commit()
